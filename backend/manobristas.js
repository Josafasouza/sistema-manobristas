...
const deletarItemDaFila = async (req, res) => {
    ...
    try {
        ...
        // 1. DELETE: Remove o item e obtém a ordem removida
        const deleteResult = await client.query('DELETE FROM fila_ativa WHERE id = $1 RETURNING ordem', [id]);

        if (deleteResult.rowCount === 0) {
            await client.query('ROLLBACK');
            return res.status(404).json({ error: 'Item da fila não encontrado.' });
        }
        const ordemRemovida = deleteResult.rows[0].ordem;

        // 2. Reorganiza: Reduz a ordem de todos os itens posteriores
        await client.query(
            'UPDATE fila_ativa SET ordem = ordem - 1 WHERE ordem > $1 AND status_servico = $2',
            [ordemRemovida, 'espera']
        );
...
module.exports = {
    ...
    retornoManobrista,
    moverItemNaFila,
    deletarItemDaFila, // Exportando a nova função
};